...
class CombSniffer(QDialog):
    ...
    def calc(self):
        ...
        if self.radioButton_Pfix.isChecked():
            ...
            Prange = list(range(self.spinBox_P1.value(), self.spinBox_P2.value()+1, 2))
    ...
    def update_data_in_gui(self):
        self.update_plot()
        self.fill_table()
        self.textBrowser_wdginfo.setHtml('{} valid windings found!<br><br>Click on a point in the \
        graphic window to get more informations!'.format(len(self.wdg_list)))
    ...
    def combination_selected_in_table(self, sel):
        ...
        for k1 in range(np.shape(l)[0]):
            for k2 in range(np.shape(l)[1]):
                ...
                table.item(k1,k2).setFlags(QtCore.Qt.ItemIsSelectable | QtCore.Qt.ItemIsEnabled)
    ...
    def fill_table(self):
        ...
        for i, key in enumerate(self.plot_keys):
            ...
            for k in range(len(self.data[key])):
                ...
                table.item(k, i).setFlags(QtCore.Qt.ItemIsSelectable | QtCore.Qt.ItemIsEnabled) # note editable
